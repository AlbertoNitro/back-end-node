...
export class DtoService {
    ...
    public static toFriendsOutputDto(unit: Unit, topUnits: Unit[], lowerUnits: Unit[], relations: Relation[]): FriendsOutputDto {
        let friendsOutputDto: FriendsOutputDto = undefined;
        if (unit) {
            friendsOutputDto = {unit: DtoService.toUnitOutputDto(unit), topUnits: DtoService.toArrayUnitOutputDto(topUnits), lowerUnits: DtoService.toArrayUnitOutputDto(lowerUnits), relations: DtoService.toArrayRelationOutputDto(relations)};
        }
        return friendsOutputDto;
    }
}
